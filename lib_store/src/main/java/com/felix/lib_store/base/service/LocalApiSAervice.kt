...
interface LocalApiSAervice {
    ...
    fun getCategoryList(): Observable<List<AppCategory>> =
        Observable.just(str.fromJson<List<AppCategory>>())
    ...
    fun update() {
        Observable.range(0, 30).map {
            AppCategory().apply {
                name =
                    ApiDelegate.getCategorTest(it).runCatching { blockingFirst() }.getOrNull() ?: ""
                id = it.toString()
            }
